...
\code{expToStringOps(}\linebreak\code{str1).+(str2)}, which evaluates to \code{StringConcat(str1, str2)} as desired.
...
To be able to inspect a HOAS function body \code{funBody: Exp[S] =>}\linebreak\code{Exp[T]}, like \code{str => str + "!"}, we convert it to \emph{first-order} abstract syntax (FOAS), that is to an expression tree of type \code{Exp[T]}.

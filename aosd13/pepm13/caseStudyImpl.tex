...
In our example, operator \code{+} should be available on \code{Exp[String]}, but not on \code{Exp[Boolean]}, because \code{+} is available on \code{String} but not on \code{Boolean}.
...
\code{expToStringOps(}\codebreak\code{str1).+(str2)}, which evaluates to \code{StringConcat(str1, str2)} as desired.
...
However, string concatenation might also be applied to constants, as in \code{str1 +}\codebreakSpace\code{"foo"} or \code{"bar" + str1}.
...
\code{str1}\linebreak\code{+ "foo" + str2} to \code{expToStringOps(str1) + pure("foo") + str2}.
...
Implicit conversions for user-defined types cooperate with other ones; for instance, \code{author.}\codebreak\code{firstName + " " + author.lastName} is rewritten to
\begin{lstlisting}
...
To be able to inspect a HOAS function body \code{funBody: Exp[S] =>}\codebreakSpace\code{Exp[T]}, like \code{str => str + "!"}, we convert it to \emph{first-order} abstract syntax (FOAS), that is to an expression tree of type \code{Exp[T]}.

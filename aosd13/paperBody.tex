...
\code{idxByPublisher(}\codebreak\code{"Pearson Education")}, only the cached result of evaluating the index is used when
...
function arguments; so for instance \code{List(1, 2, 3).map(}\codebreak\code{x => x + 1)} becomes

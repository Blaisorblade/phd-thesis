...
%
\rulename{SVar} looks up in~$\sectx$ to return the closed value of a
variable~$\tx$.
%
\rulename{STuple} looks~$\many\tx$ up in~$\sectx$ to evaluate
tuple~$\stuple{\many\tx}$, binds it to~$\ty$ and continues with
the next term.
%
...
Nil and replacement changes can be used to update
...
is defined via some interpretation function~$\delta_{\oplus}$. A

...
\item in \cref{part:static}, we extend ILC with a further program transformation
  ...
  needing to look results up at runtime. To this end, we extend techniques
  mentioned by \citet{Liu00} to a higher-order, typed setting.
\end{itemize}

\pg{rewrite this once we have a second part.}
\cref{part:incr}
The second part makes the following contributions:
\begin{itemize}
\item We present a novel mathematical theory of changes and derivatives, which is more
  ...
  they are defined also for functions.\pg{readd some reference.}
...
I did most of the implementation work: among other things I devised the
embedding for collection operations, implemented optimizations and indexing,
implemented interpretation when compilation did not work well, and performed the
performance evaluation.
% Michael Eichberg and Ralf Mitschke contributed to the
% evaluation on FindBugs code.
% Christian K{\"{a}}stner contributed, among other things,
% to the experiment design for the performance evaluation.
% Klaus Ostermann proposed the original idea and supervised the project.
...
\Cref{part:incr} contains a novel correctness proof; its history and
contributions are discussed in \cref{sec:ilc-dev-history}.

Furthermore, \cref{part:static} comes from a new, unpublished manuscript. I
designed the overall approach, the transformation and the first case study on
sequences, defining the change structure, designing the primitives and the first
case study. Yann Régis-Gianas designed the proofs. Philipp Schuster contributed
to the evaluation.
